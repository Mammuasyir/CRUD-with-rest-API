...
class MusyrifAdapter(var musyrif: List<ResultItem?>?) :
...
override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): MyViewHolder {
    val view = LayoutInflater.from(parent.context).inflate(R.layout.item_satpam, parent, false)
    return MyViewHolder(view)
}

override fun onBindViewHolder(holder: MyViewHolder, position: Int) {
    val context = holder.itemView.context
    val item = musyrif?.get(position)

    holder.tvTitle.text = item?.nama_penerima
    holder.btnDetail.setOnClickListener {
        ...
        val UrlImage = "https://paket.siyap.co.id/storage/${item?.img}"

        Glide.with(context).load(UrlImage).into(views.findViewById<ImageView>(R.id.img_paket_dtl))
        nama.text = item?.nama_penerima
        tanggal.text = item?.tanggal_input
...
override fun getItemCount() = musyrif?.size ?: 0
}