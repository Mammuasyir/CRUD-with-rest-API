...
class SelesaiAdapter(var paket: List<ResultItem?>? = listOf()) :
    RecyclerView.Adapter<SelesaiAdapter.MyViewHolder>() {
                    ...
                    override fun onResponse(
                        ...
                        if (response.isSuccessful) {
                            if (response.body()?.status == 1) {
                                ...
                                ApiConfig.instanceRetrofit.getpaketSelesai().enqueue(object :
                                    Callback<ResponsePaket> {
                                    override fun onResponse(
                                        call: Call<ResponsePaket>,
                                        response: Response<ResponsePaket>
